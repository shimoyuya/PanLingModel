...
[cite_start]本项目基于 Unreal Engine 5.5 开发，采用 **C++ 与蓝图 (Blueprint) 协同** 的最佳实践开发模式 [cite: 3]。
[cite_start]核心架构思想为 **“组合优于继承 (Composition over Inheritance)”** 以及 **“事件驱动 (Event-Driven)”** [cite: 3, 5]：

* **组件化设计**：抛弃臃肿的角色基类，将生命值管理抽离为 `AttributeComponent`，将战斗逻辑抽离为 `CombatComponent`。玩家和敌人都可以通过“挂载”这些组件快速获得相应能力。
* [cite_start]**逻辑与表现分离**：核心数值、状态机、伤害计算、AI 寻路均由 C++ 底层处理；模型、动画、粒子特效和 UI 布局均交由蓝图处理，保证了极高的性能与可维护性 [cite: 4]。
...
* [cite_start]**事件驱动 UI**：抛弃了低效的 Event Tick 绑定 UI 方式，采用 C++ 委托 (Delegate) 广播机制。只有当血量真正发生变化时，UI 才会收到通知并更新性能开销极低 [cite: 5]。
...
* [cite_start]**`APanLingCharacter` (玩家角色类)** [cite: 14]
...
* [cite_start]**`UAttributeComponent` (属性组件)** [cite: 14]
...
* [cite_start]**`UCombatComponent` (战斗组件)** [cite: 14]
...
* [cite_start]**`APanLingWeapon` (武器基类)** [cite: 14]
...
* [cite_start]**`APanLingAIController` (AI 控制器)** [cite: 14]
...
### 1. 角色与武器蓝图
* [cite_start]**`BP_PanLingCharacter`** (继承 `APanLingCharacter`)：配置了小白人模型 (SKM_Manny)、动画蓝图 (ABP_Manny)、增强输入上下文 (IMC_Default) 以及绑定的玩家 UI 类 [cite: 16]。
* [cite_start]**`BP_Enemy`** (继承 `APanLingEnemy`)：配置了敌人的模型、默认装备的武器 (`BP_Sword`)、AI 控制器 (`BP_EnemyAIController`) 以及攻击动画蒙太奇 [cite: 16]。
* [cite_start]**`BP_Sword`** (继承 `APanLingWeapon`)：配置了武器的静态网格体（剑的模型）以及用于物理判定的 Box Collision 的具体大小和相对位置 [cite: 16]。
* [cite_start]**`BP_Chest`** (继承 `ASItemChest`)：配置了宝箱底座与盖子的模型 [cite: 16]。

### 2. 动画与输入资产
* [cite_start]**`ANS_WeaponTrace`** (AnimNotifyState 蓝图)：挂载在攻击动画蒙太奇的帧轨道上。开始时调用 `StartWeaponTrace`，结束时调用 `StopWeaponTrace`，精确框定了武器的“杀伤时间段” [cite: 16]。
* **Input Actions** (`IA_Move`, `IA_Look`, `IA_Interact`, `IA_Attack`)：定义了输入的数据类型（如 2D 轴、布尔按键），并在 `IMC_Default` 中映射了对应的键盘/鼠标按键。

### 3. AI 大脑资产
* [cite_start]**`BP_EnemyAIController`**：指定了敌人要运行的特定行为树资产 [cite: 16]。
* [cite_start]**`BB_Enemy`** (黑板 Blackboard)：AI 的内存，目前包含了一个 Object 类型的键 `TargetActor`，用于记忆玩家的位置 [cite: 16]。
* [cite_start]**`BT_Enemy`** (行为树 Behavior Tree)：定义了 AI 的决策树：`Sequence` -> [`寻找并移动到 TargetActor` -> `播放 Melee Attack 任务` -> `等待 (Wait) 2秒 CD`] [cite: 16]。

### 4. UI 界面蓝图
* [cite_start]**`WBP_PlayerHUD`** (继承 `UPanLingPlayerHUD`)：界面布局蓝图。包含 Canvas Panel 和一个被严格命名为 `HealthProgressBar` 的 ProgressBar 组件，并配置了血条的颜色和屏幕位置 [cite: 16]。
